...
int main (void)
{
  ...
  do
    {
     printf ("\nOperacion a realizar(Anadir, Quitar, Buscar, Listar, Salir): ");
     do
        {
          ch = getchar ();
...
/*
  La primera operacicn que hay que hacer siempre con un arbol es inicializarlo,
...
/*
  Para ver ccmo se inserta en lo arboles, veamos ccmo se insertar¡an
...
/*
  ...
  Devuelve TRUE si se borrc el elemento o FALSE en caso contrario.
*/
...
int quitar_de_arbol (nodo **parb, int info)
{
 ...
 int encontrado = 0;
  ...
  if (encontrado) /* llama a la funcion suprimir con puntero apuntado a nodo a quitar */
    if (p == *parb)
...
/*
  ...
  Si el arbol est  ordenado, este recorrido imprime los elmentos ordenados.
*/
